...
### 5.a
Para encontrar la parte que nos correspondía dentro del archivo, escribimos un programa que recorre el archivo byte por byte buscando el nombre de nuestro grupo (las primeras 5 letras de "bitbros", o sea "bitbr") al principio de un bloque. Como el archivo tiene mucho contenido de relleno sin sentido entre los datos reales, tuvimos que ir chequeando de a poco hasta encontrar el lugar exacto donde aparecía ese "bitbr". Una vez encontrado, ahí mismo venía indicado en qué posición del mensaje total iba nuestra parte y cuántos caracteres teníamos que tomar. Siguiendo esa indicación, extrajimos el pedacito que nos tocaba: la letra "p".

### 5.b
Cuando juntamos nuestra parte con las de los demás grupos y las ordenamos según el lugar que le correspondía a cada una, nos quedó lo siguiente: tps://wm/shorts/_lLnww. Se nota que hay huecos: hay tramos del mensaje que no llegamos a completar porque, hasta el momento, no todos los grupos habían aportado su parte. Aun así, con lo que se pudo armar ya se intuye bastante bien de qué se trata: todo indica que es un link de un video de YouTube Shorts, y que las partes faltantes corresponden justo a las letras que completarían el "https://" al principio y el resto del identificador del video. Es decir, el mensaje está casi armado, solo falta que se sumen los grupos que tienen las piezas que a nosotros nos faltan.